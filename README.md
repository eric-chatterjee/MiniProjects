...
More generally, we note that since each Grover cycle rotates the composite state by $2\phi$, the optimal number of cycles will yield a state with a maximum separation of $\phi$ from the ideal angle $\pi/2$. Since $\phi = \textrm{sin}^{-1}(1/\sqrt{2^n})$, this means that in the final superposition, the overall amplitude of the "wrong" states is no more than $1/\sqrt{2^n}$, resulting in an error probability of no more than $1/2^n$ (i.e., the measurement error is $0(1/2^n)$, as desired).
